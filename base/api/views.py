...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def get_range_org_functional_level(request, start, end):
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def get_range_currencies(request, start, end):
...
@permission_classes([IsAuthenticated])
@permission_classes([IsAuthenticated])
def get_range_config_codes(request, start, end):
...
def get_range_perspectives(request, start, end):
    pers_len = perspectives.objects.filter(delete_flag="N").count()
    pers = perspectives.objects.filter(delete_flag="N")[start:end]
    pers_csv_export = perspectives.objects.filter(delete_flag="N")
    serializer = perspectives_serializer(pers, many=True)
